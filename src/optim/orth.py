...
def _XXT(A: Tensor, out: Tensor) -> Tensor:
    out.copy_(torch.matmul(A, A.transpose(-1, -2)))
    return out


def _XTX(A: Tensor, out: Tensor) -> Tensor:
    out.copy_(torch.matmul(A.transpose(-1, -2), A))
    return out


def _ba_plus_cAA(A: Tensor, alpha: float, beta: float, out: Tensor) -> Tensor:
    out.copy_(alpha * torch.matmul(A, A) + beta * A)
    return out


def make_orthogonalize_fn(
    ...
    def orthogonalize(
        ...
        if is_tall:
            ...
            for a, b, c in coeffs:
                _XTX(x, out=a_buf)
                _ba_plus_cAA(a_buf, alpha=c, beta=b, out=b_buf)
                if split_baddbmm:
