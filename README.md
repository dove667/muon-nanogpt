...
研究 Muon 优化器中 Newton-Schulz 正交化系数调度对 GPT 预训练的影响。当前实验对比五种固定配置：`adamw`、`vanilla`、`manual`、`fast`、`polar_express`，并在统一的 fixed-stack（固定 batch/seq_len/window，warmup+cosine LR）下比较验证损失、计算开销和更新矩阵的正交性。
